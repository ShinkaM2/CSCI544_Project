...
# path = r'C:\Users\mishr\OneDrive\Desktop\NeuralDecipher\data'
def probabilities(path):
    ...
    c=c[c[columns[0]]!='_']
    c=c[c[columns[1]]!='_']
    u=c[columns[0]].tolist()
    h=c[columns[1]].tolist()
    pairs=[]
    for i,j in zip(u,h):
        newi=i.split('|')
        newj=j.split('|')
    ...
    prob={}
    for i in t:
        prob[i]=0.5+(t[i].count(most_common(t[i]))/len(t[i]))
    return prob
...
class UniversalCharEmbedding(nn.Module):
    ...
    def __init__(self, langs, char_emb_dim, universal_charset_size, mapping_temperature=0.0):
        ...
        self.prob = probabilities(r'C:\Users\mishr\OneDrive\Desktop\NeuralDecipher\data\uga-heb.small.no_spe.cog')
    ...
    def mapping(self, lang):
        ...
        if weight.size()[0]==34:
            ...
            for w in weight:
                i+=1
                try:
                    char=self.charsets[lang]._CHARS[i]
                    if char in self.prob:
                        p=self.prob[char]
                    else:
                        p=1
                except:
